...
export default function YearCards({ paperId }: { paperId: string }) {
    ...
    return (
        <Card className="mt-5">
            <CardHeader>
                <CardTitle>{loading ? <Skeleton className="h-10 w-[300px] text-md" /> : <Link className='hover:underline transition-all' rel="noopener noreferrer" target="_blank" href={`/paper?paperId=${paperDetails?.id}`}>{customTrim(paperDetails?.title, 100)}</Link>}</CardTitle>
                <div>
            ...
            <CardContent>
                {loading ? (<div >
                    ...
                    <Skeleton className='h-3 w-full mt-2' />
                    <Skeleton className='h-3 w-full mt-2' />
                </div>) : <p className='text-sm'>{customTrim(paperDetails?.abstract, 250)}</p>
                }
            </CardContent>
            <CardFooter>
                <Button asChild>
